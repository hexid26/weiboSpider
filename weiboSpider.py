...
class Weibo(object):
  ...
  def __init__(self, config):
    ...
    self.write_mode = config['write_mode']  # 结果信息保存类型，为list形式，可包含txt、csv、json、mongo和mysql五种类型
    self.pic_download = config['pic_download']  # 取值范围为0、1,程序默认值为0,代表不下载微博原始图片,1代表下载
    self.video_download = config['video_download']  # 取值范围为0、1,程序默认为0,代表不下载微博视频,1代表下载
      ...
      if first_pic in a_list:
        if all_pic in a_list:
          ...
          if info.xpath('.//img/@src'):
            ...
            sys.exit(u"爬虫微博可能被设置成了'不显示图片'，请前往" u"'https://weibo.cn/account/customize/pic'，修改为'显示'")
      else:
        picture_urls = u'无'
      return picture_urls
    except Exception as e:
      ...
      if is_original:
        ...
        for a in a_list:
          if 'm.weibo.cn/s/video/show?object_id=' in a.xpath('@href')[0]:
            ...
            break
        if video_link != u'无':
          video_link = video_link.replace('m.weibo.cn/s/video/show', 'm.weibo.cn/s/video/object')
  ...
  def get_one_page(self, page):
    """获取第page页的全部微博"""
    try:
      url = 'https://weibo.cn/%s?page=%d' % (self.user_config['user_uri'], page)
      selector = self.handle_html(url)
          ...
          if weibo:
            ...
            if publish_time < since_date:
              if self.is_pinned_weibo(info[i]):
                ...
                print(u'{}已获取{}({})的第{}页微博{}'.format('-' * 30, self.user['nickname'],
            ...
            self.got_num += 1
            print('-' * 100)
      print(u'{}已获取{}({})的第{}页微博{}'.format('-' * 30, self.user['nickname'], self.user['id'], page,
                                           '-' * 30))
    except Exception as e:
      print('Error: ', e)
      traceback.print_exc()
  ...
  def get_filepath(self, type):
    ...
    try:
      file_dir = os.path.split(
          os.path.realpath(__file__))[0] + os.sep + 'weibo' + os.sep + self.user['nickname']
  ...
  def write_log(self):
    ...
    file_dir = os.path.split(os.path.realpath(__file__))[0] + os.sep + 'weibo' + os.sep
    if not os.path.isdir(file_dir):
      os.makedirs(file_dir)
    ...
    try:
      ...
      if wrote_num == 0:
        ...
        result_header = (u'用户信息\n用户昵称：' + self.user['nickname'] + u'\n用户id: ' +
                         str(self.user_config['user_id']) + u'\n微博数: ' +
                         str(self.user['weibo_num']) + u'\n关注数: ' + str(self.user['following']) +
                         u'\n粉丝数: ' + str(self.user['followers']) + result_header)
      ...
      for i, w in enumerate(self.weibo[wrote_num:]):
        temp_result.append(
            ...
            u'   转发数: ' + str(w['retweet_num']) + u'   评论数: ' + str(w['comment_num']) + '\n' +
      ...
      for i, line in enumerate(lines):
        ...
        if len(info) > 0 and info[0].isdigit():
          if self.user_config['user_uri'] == info[0]:
            ...
            if len(info) == 2:
              info.append(self.start_time)
            if len(info) > 3 and self.is_date(info[2] + ' ' + info[3]):
              del info[3]
            if len(info) > 2:
...
def main():
  global record_time
  try:
    time_path = os.path.split(os.path.realpath(__file__))[0] + os.sep + 'time.txt'
    ...
    with open(config_path) as f:
      try:
        ...
        sys.exit(u'config.json 格式不正确，请参考 ' u'https://github.com/dataabc/weiboSpider#3程序设置')
    wb = Weibo(config)
    wb.start()  # 爬取微博信息
